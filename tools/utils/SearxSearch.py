...
class Filters(BaseModel):
    ...
    def compile_filters(self) -> str:
        ...
        if self.exact_phrases:
            phrase_list = (
                self.exact_phrases
                if isinstance(self.exact_phrases, list)
                else [self.exact_phrases]
            )
...
def search(
    ...
    Args:
        ...
        category (optional): Category to search in ('general', 'news', 'images', 'videos')
        language (optional): Language code for search results
        safe (optional): Safe search level (0: None, 1: Moderate, 2: Strict)
    ...
    if filters:
        params["q"] = f"{query} {filters.compile_filters()}"
