...
from spider_rs import Page, Website
...
logging.basicConfig(
    # filename=f"logs/scrape_website_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log",
    # filemode="a",
    level=logging.INFO,
    handlers=[handler],
)
...
LLM_MODEL = "phi"
LLM_MODEL = "llamascout"
SUMMARIZATION_MODEL = "openai-fast"
BASE_URL = os.getenv("POLLINATIONS_BASE_URL")
...
EXCLUDE_PATTERNS = [
    ...
    r"/(settings|preferences|config|admin|dashboard)/?",  # config/user settings
...
def soup_html(html: str, baseurl: str = "") -> tuple[str, list[str]]:
    """
    HTML Cleanup Magic 🪄
...
def html2md(html: str, instructions: str = "") -> str:
    """
    ...
    try:
        ...
        if len(html) > 400000:
            logger.info("[html2md] HTML TRUNCATED to 400000")
            html = html[:400000]
...
def summarize_content(content: str, instructions: str = "") -> str:
    """
    ...
    try:
        ...
        if not instructions:
            instructions = """You are a precise and context-aware summarization assistant. 
    ...
    try:
        ...
        if "pdf" in url:
            ...
            if use_reader_lm:
                ...
                page.markdown = html2text(
                    html=page.cleaned_html, bodywidth=1000
                ).strip()
            ...
            if (
                ...
                or len(page.markdown) < len(page.cleaned_html) * 0.4
            ):
                page.markdown = jina_reader_api(url=url)
...
if __name__ == "__main__":
    ...
    page_result = scrape_page(url=url, summarise=True)
    print(f"URL: {page_result.url}")
    print(f"Markdown: {page_result.markdown}")
    print(f"Summary: {page_result.summary}")
    print(f"{len(page_result.links)} links found.")
