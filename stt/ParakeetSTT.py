"""
The stt class in the provided code facilitates
fast speech-to-text transcription.

The class employs the onnx_asr library to transcribe the recorded audio
...
INIT_MODEL_TRANSCRIPTION = "nemo-parakeet-tdt-0.6b-v3"
INIT_MODEL_TRANSCRIPTION_REALTIME = "nemo-parakeet-tdt-0.6b-v3"
...
PARAKEET_MODEL_PATH = MODELS_PATH / "parakeet"
OWW_MODEL_PATH = MODELS_PATH / "oww"
SILERO_MODEL_PATH = MODELS_PATH / "silero"
...
class TranscriptionWorker:
    ...
    def run(self):
        ...
        logging.info(
            f"Initializing onnx_asr main transcription model {self.model_name}"
        )
        ...
        try:
            ...
            model.recognize(waveform=dummy_audio, sample_rate=16000, language="en")
        except Exception as e:
            logging.exception(
                f"Error initializing main onnx_asr transcription model: {e}"
            )
            raise

        self.ready_event.set()
        logging.debug(
            "onnx_asr main speech to text transcription model initialized successfully"
        )
...
class stt:
    ...
    def __init__(
        ...
        download_root: Path | str | None = PARAKEET_MODEL_PATH,
        language: str = "en",
        compute_type: str = "",
        ...
        openwakeword_model_paths: List[str] = [str(OWW_MODEL_PATH / "hey_jarvis.onnx")],
        ...
        Args:
        - model (str, default="nemo-asr-tdt-0.6b-v3"): Specifies the transcription model to use or the path to a converted model directory.
        - download_root (str, default=None): Specifies the root path where the Whisper models are downloaded to. When empty, the default is used.
        - language (str, default="en"): Language code for speech-to-text engine.
        - compute_type (str, default="int8"): Specifies the type of computation to be used for transcription. ["int8", "default"]
        - input_device_index (int, default=0): The index of the audio input device to use.
        ...
        - use_microphone (bool, default=True): Specifies whether to use the microphone as the audio input source. If set to False, the audio input source will be the audio data sent through the feed_audio() method.
        ...
        - init_realtime_after_seconds (float, default=0.2): Specifies the initial waiting time after the recording was initiated before yielding the first realtime transcription
        ...
        - post_speech_silence_duration (float, default=0.5): Duration in seconds of silence that must follow speech before the recording is considered to be completed. This ensures that any brief pauses during speech don't prematurely end the recording.
        ...
        - pre_recording_buffer_duration (float, default=0.5): Duration in seconds for the audio buffer to maintain pre-roll audio (compensates speech activity detection latency).
        ...
        - wakeword_backend (str, default=""): Specifies the backend library to use for wake word detection. Supported options include 'oww' for the OpenWakeWord engine.
        - openwakeword_model_paths (List[str], default=None): Comma-separated paths to model files for the openwakeword library. These paths point to custom models that can be used for wake word detection when the openwakeword library is selected as the wakeword_backend.
        ...
        - wake_word (str, default=""): wake word to initiate recording. When using the 'openwakeword' backend, wake words are automatically extracted from the provided model files, so specifying them here is not necessary.
        ...
        - wake_word_buffer_duration (float, default=0.5): Duration in seconds to buffer audio data during wake word detection. This helps in cutting out the wake word from the recording buffer so it does not falsely get detected along with the following spoken text, ensuring cleaner and more accurate transcription start triggers. Increase this if parts of the wake word get detected as text.
        ...
        - on_recorded_chunk (callable, default=None): Callback function to be called when a chunk of audio is recorded. The function is called with the recorded audio chunk as its argument.
        - debug_mode (bool, default=False): If set to True, the system will print additional debug information to the console.
        - handle_buffer_overflow (bool, default=True): If set to True, the system will log a warning when an input overflow occurs during recording and remove the data from the buffer.
        - buffer_size (int, default=512): The buffer size to use for audio recording. Changing this may break functionality.
        - sample_rate (int, default=16000): The sample rate to use for audio recording.
        - print_transcription_time (bool, default=False): Logs processing time of main model transcription.
        - early_transcription_on_silence (int, default=0): If set, the system will transcribe audio faster when silence is detected. Transcription will start after the specified milliseconds, so keep this value lower than post_speech_silence_duration.
            Ideally around post_speech_silence_duration minus the estimated transcription time with the main model. If silence lasts longer than post_speech_silence_duration, the recording is stopped, and the transcription is submitted. If voice activity resumes within this period, the transcription is discarded. Results in faster final transcriptions to the cost of additional GPU load due to some unnecessary final transcriptions.
        - allowed_latency_limit (int, default=100): Maximal amount of chunks that can be unprocessed in queue before discarding chunks.
        - no_log_file (bool, default=True): Skips writing of debug log file.
        - use_extended_logging (bool, default=False): Writes extensive log messages for the recording worker, that processes the audio chunks.
        ...
        self.compute_type = "" if compute_type == "default" else compute_type
        ...
        if not download_root:
            download_root = None  # type: ignore
        self.download_root = download_root
        ...
        self.use_wake_words = wakeword_backend in {
            "oww",
            "openwakeword",
            "openwakewords",
        }
        ...
        log_format = "Real-time STT: %(name)s - %(levelname)s - %(message)s"
        ...
        if not no_log_file:
            ...
            file_handler = logging.FileHandler("Real-time STT.log")
        ...
        logging.info("\n\nStarting Real-time STT")

        if use_extended_logging:
            logging.info("Real-time STT was called with these parameters:")
        ...
        if self.enable_realtime_transcription and not self.use_main_model_for_realtime:
            try:
                ...
                self.realtime_model_type = onnx_asr.load_model(
                    model=self.realtime_model_type,
                    path=self.download_root,
                    quantization=self.compute_type,
                )
                dummy_audio = np.zeros(16000, dtype=np.float32)
                self.realtime_model_type.recognize(
                    waveform=dummy_audio, sample_rate=16000, language="en"
                )
            except Exception as e:
                logging.exception(
        ...
        if wakeword_backend in ["oww", "openwakeword", "openwakewords"]:
            ...
            if self.wakeword_backend in ["oww", "openwakeword", "openwakewords"]:
                ...
                logging.exception(
                    f"Wakeword engine {self.wakeword_backend} unknown/unsupported. Please specify one of: openwakeword."
                )

        # Setup voice activity detection with silero
        try:
            ...
            try:
                ...
                logging.info("Silero VAD module not available. Checking local path...")
        ...
        logging.debug("Real-time STT initialization completed successfully")
    ...
    def _audio_data_worker(
        ...
        chunk_size = 2048  # Increased chunk size for better performance
    ...
    def wakeup(self):
        """
        If in wake word mode, wake up as if a wake word was spoken.
        """
    ...
    def shutdown(self):
        ...
        with self.shutdown_lock:
            ...
            # print("\n\033[91mReal-time STT shutting down\033[0m")
            # logging.debug("Real-time STT shutting down")
                        ...
                        with self.transcription_lock:
                            try:
                                ...
                                if self.parent_transcription_pipe.poll(
                                    ...
                                    if status == "success":
                                        logging.debug(
                                            f"Realtime text detected with main model: {result}"
                                        )
                                    else:
                                        logging.error(
